 ...
 */
commitProcessingQueue.process('commit-processing', 5, async (job) => {
    ...
    try {
        ...
        // Check rate limit
        job.log('Checking GitHub rate limit...');
        const canMakeRequest = await RateLimitService.checkCanMakeRequest(userId);

        if (!canMakeRequest) {
            job.log('Rate limit approached, waiting for reset...');
        ...
        const commitDoc = new Commit({
            ...
            authorName: commitDetails.author?.name || 'Unknown',
            authorEmail: commitDetails.author?.email || 'unknown@example.com',
            url: commitDetails.url || '',
            branch: commitDetails.branch || 'unknown',
